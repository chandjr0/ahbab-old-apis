...
const test = async (req, res) => {
  const orderData = await OrderModel.aggregate([
    {
      $match: {
        "orderStatus.status": { $nin: ["CANCELED", "RETURNED"] },
    ...
    {
      $match: {
        "products.productId": ObjectId("64abebd717334b5ae471666d"),
      },
    },
    // {
    //   $project: {
    ...
    // {
    //   $match: {
    //     // "products.isVariant": false,
    //     "products.isVariant": true,
    //   },
    // },
  ...
  return res.json({ msg: "hi", data: orderData, len: orderData.length });
};
